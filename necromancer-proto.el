...
(defvar necromancer--input-manner-map
  (make-sparse-keymap "Include context: [x]none [c]ontext [p]reamble [w]hole [RET]accept_defaults"))

(define-key necromancer--input-manner-map (kbd "x")
  (lambda ()
    (interactive)
    (setq necromancer--input-manner -1)
    (force-mode-line-update t)
    (message "No extra context. Just selected region & task, when provided")))
...
(define-key necromancer--input-manner-map (kbd "w")
  (lambda ()
    (interactive)
    (setq necromancer--input-manner 2)
    (force-mode-line-update t)
    (message "Include goal + context + whole source doc")))
...
(defvar necromancer--output-manner-map
  (make-sparse-keymap "Insert responses: [^]above [!]overwrite [.]below [>]eof [s]idebar [DEL]default/no_op"))
  ...
  (if necromancer-mode
      (format " Necro:%s/%s;%s/%s (%s)"
              ...
              (cond
               ...
               ((equal necromancer--input-manner 2)  "w"))
              (cond
               ((equal necromancer--output-manner -1) "^")
               ((equal necromancer--output-manner 0)  "!")
               ((equal necromancer--output-manner 1)  ".")
               ((equal necromancer--output-manner 2)  ">")
